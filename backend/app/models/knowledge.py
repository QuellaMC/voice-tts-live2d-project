...
class Knowledge(Base):
    ...
    # Relationships
    tags = relationship(
        "Tag", secondary=knowledge_tags, back_populates="knowledge_items"
    )
    concepts = relationship(
        "Concept", secondary=knowledge_concepts, back_populates="knowledge_items"
    )
    audit_logs = relationship(
        "KnowledgeAudit", back_populates="knowledge", cascade="all, delete-orphan"
    )
...
class Tag(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("name ~ '^[a-zA-Z0-9_-]+$'", name="tag_name_format_check"),
...
class Concept(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("name ~ '^[a-zA-Z0-9_-]+$'", name="concept_name_format_check"),
        {"extend_existing": True},
    )
...
class KnowledgeAudit(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    knowledge_id = Column(
        Integer, ForeignKey("knowledge.id", ondelete="CASCADE"), nullable=False
    )
    user_id = Column(
