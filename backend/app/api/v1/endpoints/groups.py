...
class PermissionSet(BaseModel):
    ...
    resource_type: str = Field(
        ..., description="Type of resource (knowledge, concept, tag)"
    )
    resource_id: Optional[int] = Field(
        None, description="Specific resource ID or null for all"
    )
    can_read: bool = True
...
async def update_group(
    group_id: int, data: GroupUpdate, current_user: str = Depends(get_current_admin)
):
    """Update a user group.
...
async def add_users_to_group(
    group_id: int, user_ids: List[int], current_user: str = Depends(get_current_admin)
):
    ...
    for user_id in user_ids:
        get_permission_service().add_user_to_group(
            user_id=user_id, group_id=group_id, added_by=current_user.id
        )
