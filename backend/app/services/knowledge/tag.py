...
class TagService(BaseService[Tag, TagCreate, TagUpdate]):
    ...
    async def create_with_validation(
        ...
        try:
            ...
            if existing:
                raise HTTPException(
                    status_code=400, detail=f"Tag '{obj_in.name}' already exists"
                )

            # Create tag
            db_obj = Tag(
                name=obj_in.name, description=obj_in.description, created_by=user_id
            )
            db.add(db_obj)
            db.commit()
            db.refresh(db_obj)
    ...
    async def update_with_validation(
        self, db: Session, *, db_obj: Tag, obj_in: TagUpdate, user_id: int
    ) -> Tag:
    ...
    async def get_or_create(
        self, db: Session, *, name: str, description: Optional[str] = None, user_id: int
    ) -> Tag:
