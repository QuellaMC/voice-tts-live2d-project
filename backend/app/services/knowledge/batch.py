...
from app.models.knowledge import Knowledge, Tag, Concept, knowledge_tags, knowledge_concepts
...
class KnowledgeBatchService(IKnowledgeBatchService):
    ...
    async def create_many(
        ...
        *,
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to create knowledge entries in batch"
            )
    ...
    async def cleanup_orphaned(self, db: Session) -> Dict[str, int]:
        ...
        try:
            # Find orphaned tags
            orphaned_tags = db.query(Tag).outerjoin(
                knowledge_tags
            ).filter(
            ...
            orphaned_concepts = db.query(Concept).outerjoin(
                knowledge_concepts
            ).filter(
                knowledge_concepts.c.concept_id.is_(None)
            ).filter(
                Concept.children.any() == False
            ).all()
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to clean up orphaned items"
            )
    ...
    async def reindex_embeddings(
        ...
        *,
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to reindex embeddings"
            ) 