...
def test_synthesize_speech(client, user_headers, mock_tts_service):
    ...
    if len(voices) > 0:
        ...
        response = client.post("/api/v1/tts/synthesize", json=synth_data, headers=user_headers)
        assert response.status_code == 200
        data = response.json()
...
def test_update_tts_settings(client, user_headers, mock_tts_service):
    ...
    response = client.put("/api/v1/tts/settings", json=settings_data, headers=user_headers)
    assert response.status_code == 200
    data = response.json()
